...
export default function AnalysisPage() {
  ...
  return <AppShell active="analysis"><div className="page-width page-stack">
    <header className="analysis-header"><div><div className="analysis-breadcrumb"><Link href="/">ClassTrace</Link><span>/</span><span>Circle area scaling</span></div><h1>Class reasoning analysis</h1><p>Prepared demo · analysis complete</p></div><div className="analysis-actions"><ButtonLink href="/interventions/demo" variant="secondary">Preview intervention</ButtonLink><ButtonLink href="/analyses/demo/outcomes">View outcomes <ArrowRight size={16} /></ButtonLink></div></header>
    ...
    <TraceMap />
    <section><div className="section-heading-row"><div><span className="eyebrow">Learning-need groups</span><h2>Misconception clusters</h2></div><p>Ordered by instructional leverage, not score.</p></div><div className="cluster-grid">{clusters.map((cluster) => <ClusterCard key={cluster.id} cluster={cluster} />)}</div></section>
    <div className="analysis-lower"><section className="attention-queue"><div className="section-heading-row"><div><span className="eyebrow">Teacher attention</span><h2>Three interpretations to check</h2></div><Badge tone="amber">Human review</Badge></div><div>{attention.map((item) => { const response = getResponse(item.responseId); const student = response ? getStudent(response.studentId) : undefined; return response && student ? <Link key={item.responseId} href={`/analyses/demo/clusters/${item.clusterId}`}><span className="student-monogram">{student.label.slice(-2)}</span><span><strong>{student.label}</strong><small>{item.summary}</small></span><Badge tone="amber">{item.confidence}</Badge><ArrowRight size={16} /></Link> : null; })}</div></section><section className="pipeline-card"><span className="eyebrow">Analysis record</span><h2>How this trace was built</h2><AnalysisPipeline /></section></div>
  </div></AppShell>;
}
