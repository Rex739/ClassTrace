...
export function LiveAnalysisDashboard() {
  ...
  return <AppShell active="analysis"><div className="page-width page-stack">
    ...
    <section className="attention-queue"><div className="section-heading-row"><div><span className="eyebrow">Teacher attention</span><h2>{attention.length} responses need review</h2></div><Badge tone="amber">Teacher review required</Badge></div><div>{attention.map((item) => <Link key={item.responseId} href="/analyses/live"><span className="student-monogram">{item.studentAlias.slice(-2)}</span><span><strong>{item.studentAlias}</strong><small>{item.reviewReason ?? item.observableReasoningSummary}</small></span><Badge tone="amber">{Math.round(item.confidence * 100)}%</Badge></Link>)}</div></section>
    <ProvenanceCard run={{ ...run, classAnalysis }} />
  </div></AppShell>;
}
